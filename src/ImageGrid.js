...
function ImageGrid({ accessToken }) {
    ...
    const handleImageClick = (image, index) => {
        //console.log(axios.get('https://www.googleapis.com/drive/v3/files/'+image.id,{headers:{Authorization: `Bearer ${accessToken}`,}}).data.files)
        getImage(image.id);
    ...
    useEffect(() => {
        const fetchImages = async () => {
            try {
                const folderId = '1CBwGeBCHalqa1-9K6oR5Us7XqzhuseFs';
                const response = await axios.get('https://www.googleapis.com/drive/v3/files', {
                    ...
                    params: {
                        ...
                        //q: ` trashed=false AND (mimeType='image/jpeg' OR mimeType='image/png' OR mimeType='image/gif' OR mimeType='image/bmp' OR mimeType='image/webp')`,
